...
- **Pendiente de decidir con el equipo: muestreo continuo (lo que ya usa `SpectrumSampler`/`PrimaryGeneratorAction` hoy) vs. bins monoenergéticos + reponderación.** Es urgente independientemente de la decisión del Halbach (punto de más abajo): cada corrida de SEP/GCR lanzada hoy con muestreo continuo cuenta eventos de forma incompatible con un esquema de bins (25 energías fijas × N eventos cada una, combinadas con peso al final) — no es ajustar un parámetro, cambiaría cómo `PrimaryGeneratorAction` genera partículas y cómo se agregan los resultados. El esquema por bins da mejor estadística en las colas del espectro (energías raras pero de alto impacto en dosis por órgano) y es el que usó CREW HaT para su criterio de error <5%; el muestreo continuo es más simple y ya funciona, pero es débil justo para dosis por órgano en energías poco frecuentes — que es lo que el equipo ya decidió reportar (ICRP110). Mientras no se decida, cualquier corrida de producción lanzada con el esquema actual arriesga no ser reusable si el equipo adopta bins después.
